...
import type { HookConfig } from "../types/provider.js";
...
export class HookManager {
  ...
  async execute(event: EventName, context: HookContext): Promise<HookResult[]> {
    const results: HookResult[] = [];
    // 循环hooks
    for (const hook of this.hooks) {
      //如果hook的event不存在，则直接跳过
  ...
  async firePreToolHooks(
    ...
    const results = await this.execute("pre_tool_use", context);
      ...
      case "command": {
        ...
        try {
          ...
          const output = execSync(command, {
            ...
            env: {
              ...process.env,
              NUOMI_EVENT: context.event,
              NUOMI_TOOL: context.toolName ?? "",
              // 注入文件路径环境变量，与 Go 版保持一致
              NUOMI_FILE_PATH: context.filePath ?? "",
            },
          });
...
export function validateHooks(hooks: HookConfig[]): Error | null {
