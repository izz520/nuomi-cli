...
export function systemSection(): Section {
  return {
    ...
    content:
      ...
      "\n# 持久记忆\n" +
      " - 主 Agent 应主动维护持久记忆：任务需要既有背景时先用 ReadMemory 按需读取；" +
      "发现经过验证、跨会话仍有价值的用户偏好、项目约定或关键事实时，用 WriteMemory 或 EditMemory 更新。\n" +
      " - MEMORY.md 只作为简洁索引，详细内容放到主题文件。" +
      "不要记录秘密、未经确认的推测、临时任务状态或可从代码直接得到的信息。\n" +
      " - 新信息与旧记忆冲突时，以最新的已验证事实为准并修正旧记忆。",
  };
}
